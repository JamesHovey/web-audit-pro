    ...
    case "traffic":
      return (
        <div className="space-y-6">
          ...
          {results.internalLinkAnalysis?.orphanedSitemapPages && results.internalLinkAnalysis.orphanedSitemapPages.length > 0 && (
            <div id="orphaned-sitemap-table" data-subsection="orphaned-sitemap" className="mb-6">
              <h4 className="font-semibold mb-3 text-red-600">
                🚨 Orphaned Pages in Sitemap ({results.internalLinkAnalysis.orphanedSitemapPages.length})
              </h4>
              <div className="bg-red-50 border border-red-200 rounded-lg overflow-hidden">
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    ...
                    <tbody className="divide-y divide-red-200">
                      {results.internalLinkAnalysis.orphanedSitemapPages.slice(0, 20).map((page: { url: string; inSitemap: boolean; incomingLinkCount: number }, index: number) => (
                        <tr key={index} className="hover:bg-red-50">
                          <td className="px-4 py-3">
                            <Tooltip content={page.url}>
                              ...
                              >
                                {page.url.replace(/^https?:\/\//, '').substring(0, 80)}
                                {page.url.length > 80 ? '...' : ''}
                              </a>
                            </Tooltip>
                          </td>
                          ...
                          <td className="px-4 py-3 text-center">
                            <span className="px-2 py-1 rounded text-xs bg-red-100 text-red-700 font-bold">
                              {page.incomingLinkCount}
                            </span>
                          </td>
                          <td className="px-4 py-3 text-gray-600">
                            Add 3-5 internal links from related pages
                          </td>
                        </tr>
                      ))}
                    </tbody>
                ...
                {results.internalLinkAnalysis.orphanedSitemapPages.length > 20 && (
                  <div className="px-4 py-2 bg-red-100 text-sm text-red-700">
                    Showing 20 of {results.internalLinkAnalysis.orphanedSitemapPages.length} orphaned sitemap pages
                  </div>
                )}
              </div>
              <p className="text-xs text-gray-600 mt-2">
                🚨 <strong>CRITICAL:</strong> These pages are in your sitemap.xml but have ZERO incoming internal links. Users cannot navigate to them, and they waste crawl budget. Add internal links from 3-5 related pages or remove them from your sitemap.
              </p>
            </div>
          )}
    ...
    case "technical":
      ...
      return (
        <div className="space-y-6">
          ...
          {/* Pages Returning 4XX/5XX Status Codes Table */}
          {results.pages404 && results.pages404.length > 0 && (
            <div id="pages-404-table" className="mb-6">
              <h4 className="font-semibold mb-3 text-red-600">❌ Pages Returning 4XX/5XX Status Codes ({results.pages404.length})</h4>
              <div className="bg-red-50 border border-red-200 rounded-lg overflow-hidden">
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead className="bg-red-100">
                    ...
                    <tbody className="divide-y divide-red-200">
                      {results.pages404.slice(0, 20).map((page: Record<string, unknown>, index: number) => (
                        <tr key={index} className="hover:bg-red-50">
                          ...
                          <td className="px-4 py-3 text-gray-600">
                            {(page.statusCode as number) >= 500 ? 'Server error - contact hosting' : (page.statusCode as number) === 403 ? 'Fix permissions or remove link' : 'Redirect or remove link'}
                          </td>
                        </tr>
                      ))}
                    </tbody>
                ...
                {results.pages404.length > 20 && (
                  <div className="px-4 py-2 bg-red-100 text-sm text-red-700">
                    Showing 20 of {results.pages404.length} pages with HTTP errors
                  </div>
                )}
              </div>
              <p className="text-xs text-gray-600 mt-2">
                💡 Tip: 4XX errors (403 Forbidden, 404 Not Found) need redirects or link removal. 5XX errors (500, 503) indicate server problems that need hosting support.
              </p>
            </div>
          )}
          ...
          {/* Duplicate Meta Descriptions and H1s Table */}
          {results.duplicateDescriptions && results.duplicateDescriptions.length > 0 && (
            <div id="duplicate-descriptions-table" className="mb-6">
              <h4 className="font-semibold mb-3 text-amber-600">🔄 Pages with Duplicate Meta Descriptions & H1 Tags ({results.duplicateDescriptions.reduce((sum: number, d: { count: number }) => sum + d.count, 0)} pages affected)</h4>
              <div className="bg-amber-50 border border-amber-200 rounded-lg overflow-hidden">
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead className="bg-amber-100">
                      <tr>
                        <th className="px-4 py-3 text-left font-medium text-amber-800">Type</th>
                        <th className="px-4 py-3 text-left font-medium text-amber-800">Duplicate Content</th>
                        <th className="px-4 py-3 text-center font-medium text-amber-800">Pages Using</th>
                        <th className="px-4 py-3 text-left font-medium text-amber-800">Affected URLs</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-amber-200">
                      {results.duplicateDescriptions.slice(0, 20).map((duplicate: { description: string; count: number; pages: Array<{ url: string }> }, index: number) => {
                        const isH1 = duplicate.description.startsWith('[H1] ');
                        const content = isH1 ? duplicate.description.substring(5) : duplicate.description;
                        const type = isH1 ? 'H1 Tag' : 'Meta Description';
                        const typeColor = isH1 ? 'text-purple-700' : 'text-blue-700';

                        return (
                          <tr key={index} className="hover:bg-amber-50">
                            <td className="px-4 py-3">
                              <span className={`font-semibold ${typeColor}`}>{type}</span>
                            </td>
                            <td className="px-4 py-3">
                ...
                {results.duplicateDescriptions.length > 20 && (
                  <div className="px-4 py-3 bg-amber-50 border-t border-amber-200 text-xs text-amber-700">
                    Showing 20 of {results.duplicateDescriptions.length} duplicate content groups
                  </div>
                )}
              </div>
              <p className="text-xs text-gray-600 mt-2">
                ⚠️ Duplicate meta descriptions and H1 tags reduce SEO effectiveness. Each page should have unique, relevant content that accurately describes what users will find. This helps search engines understand your pages better and improves rankings.
              </p>
            </div>
          )}
